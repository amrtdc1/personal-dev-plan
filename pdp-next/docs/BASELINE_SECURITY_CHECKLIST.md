...
| Server-side ownership checks | Every read/write/delete validates current user owns target record | Not Started | Needed before exposing route handlers or server mutations |
...
| Soft-delete lifecycle | 60-day retention with restore and purge windows | Not Started | Explicit migration requirement |
...
| Input validation | Validate all persisted payloads (title, dates, status enums, ordering indexes) | In Progress | Repository methods validate core fields; central schema validation still pending |
...
3. Implement soft-delete fields and retention timers in repository/data model.
