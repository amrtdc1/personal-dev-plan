...
| Server-side ownership checks | Every read/write/delete validates current user owns target record | In Progress | Protected owner-scoped goals/subgoals/tasks routes are implemented and profile writes now run through a protected server route; remaining work is broader endpoint expansion and automated route tests |
...
1. Add route-level regression tests for ownership + auth contracts across all protected endpoints (including profile write path).
2. Finalize CSP policy and lock trusted script/image/connect origins.
3. Define recurring dependency audit cadence and enforcement in CI.
4. Complete strict markdown integration for journal edit/display parity beyond migration preview.
