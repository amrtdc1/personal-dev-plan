...
| Permanent delete cascades | Cascading deletes (`deleteGoalWithChildren`, `deleteSubgoalWithTasks`, `deleteTask`) | Safe destructive workflows with ownership checks + confirm UX | Done | Archived-item permanent delete actions now use explicit confirmation and owner-scoped DELETE APIs with goal/subgoal cascade semantics |
